...
public class TextCompiler {
  ...
  public TextCompiler(final char[] document, final int templateStart, final int templateLength,
    ...
    template = document;
    end = templateStart + templateLength;
    cursor = templateStart;
  }

  private int balancedCaptureWithLineAccounting(final char[] chars, int start, final int end,
      final char type,
    ...
    if (type == term) {
      ...
      int lines = 0;
      for (start++; start < end; start++) {
        if (TextCompiler.isWhitespace(chars[start])) {
          switch (chars[start]) {
            case '\r':
              continue;
            case '\n':
    ...
    switch (type) {
      case '[':
        throw TextTemplateUtil.createCompileException(getTemplateFileName(),
            "unbalanced braces [ ... ]", chars, st, null);
      case '{':
        throw TextTemplateUtil.createCompileException(getTemplateFileName(),
            "unbalanced braces { ... }", chars, st, null);
      case '(':
      ...
      default:
        throw TextTemplateUtil.createCompileException(getTemplateFileName(),
            "unterminated string literal", chars,
            st, null);
    }
  }
  ...
  private int captureOrbToken() {
    int newStart = ++cursor;
    while ((cursor != end) && TextCompiler.isIdentifierPart(template[cursor])) {
      cursor++;
    }
    if (cursor != end) {
      if (template[cursor] == '{') {
  ...
  private int captureStringLiteral(final char type, final char[] expr, int cursor, final int end) {
    ...
    if ((cursor >= end) || (expr[cursor] != type)) {
      throw TextTemplateUtil.createCompileException(getTemplateFileName(),
          "unterminated string literal", expr,
          cursor, null);
    }
  ...
  public Node compileFrom(Node root, final ExecutionStack stack) {
        ...
        case '$':
          ...
          if ((x = captureOrbToken()) != -1) {
            ...
            switch ((opcode = OPCODES.get(name = new String(capture()))) == null ? 0 : opcode) {
              ...
              case Opcodes.CODE:
                n = markTextNode(n).next = new CodeNode(start, name, template,
                    captureOrbInternal(), start = cursor + 1, createNodeHelper());
                break;

              case Opcodes.COMMENT:
                n = markTextNode(n).next =
                    new CommentNode(start, name, template, captureOrbInternal(),
                        start = cursor + 1);
              ...
              default:
                if (name.length() == 0) {
                  ...
                  CompileException e = TextTemplateUtil.createCompileException(
                      getTemplateFileName(), "unknown token type: " + name, template, start, null);
    ...
    if (!stack.isEmpty()) {
      CompileException ce =
          TextTemplateUtil.createCompileException(getTemplateFileName(), "unclosed @"
              + ((Node) stack.peek()).getName()
              + "{} block. expected @end{}", template, cursor, null);
